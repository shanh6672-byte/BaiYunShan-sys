...
const ApiService = {
    ...
    async getWarnings() {
        if (this.USE_MOCK) return [];
        return this.request('/api/dashboard/warnings');
    },
        ...
        <div class="float-panel" id="spatialPanel">
            ...
            <div class="inner-tab" id="inner-ndvi">
                <div class="panel-card"><div class="card-header"><h3>NDVI分析</h3></div><div class="card-body">
                    ...
                    <button class="btn btn-outline btn-block" onclick="showNdviOnDashMap()" style="margin-top:6px;">显示渲染结果</button>
...
async function runFvcAnalysis() {
    ...
    // 按阈值分级渲染 FVC 并贴到地图
    if (typeof GeoServerLayers !== 'undefined') {
        GeoServerLayers.addFvcClassified(source, thresholds);
    }
...
function showNdviOnDashMap() {
    ...
    if (window._dashNdviLayer) {
        ...
        if (map && map.hasLayer(window._dashNdviLayer)) {
            map.removeLayer(window._dashNdviLayer);
        }
        window._dashNdviLayer = null;
        return;
    }
...
function runNdviAnalysis() {
    ...
    // 直接加载预生成的 NDVI 分级 PNG 到高德底图
    if (typeof GeoServerLayers !== 'undefined') {
        GeoServerLayers.showNdviOverlay('ndvi_2021_classified.png');
    }
