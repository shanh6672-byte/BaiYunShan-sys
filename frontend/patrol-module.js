...
var Patrol = {
    ...
    _loadTasks: async function () {
        try {
            ...
            var [r1, r2] = await Promise.all([
                fetch('/api/patrol-tasks'),   // 内存任务(横杠)
                fetch('/api/patrol/tasks')    // 数据库任务(斜杠)
            ]);
            var j1 = await r1.json(), j2 = await r2.json();
            var mem = (j1.success && Array.isArray(j1.data)) ? j1.data : (Array.isArray(j1) ? j1 : []);
            var db  = Array.isArray(j2) ? j2 : [];
            var seen = {}, merged = [];
            mem.concat(db).forEach(function(t) {
                var k = t.taskNumber || t.name || t.id;
                if (!seen[k]) { seen[k] = true; merged.push(t); }
            });
        ...
        try {
            ...
            if(js.success) {
                ...
                if(d.waypoints && d.waypoints.length > 1) {
                    ...
                    // 点击地图清除预览路线
                    var self2 = this;
                    var pmap = MapFacade.getMap('dashMap') || MapFacade.getMap();
                    if (pmap) {
                        var handler = function() {
                            self2._clearRoutePreview();
                            pmap.off('click', handler);
                        };
                        pmap.on('click', handler);
                    }
                }
                await this._loadRoutes();
    ...
    _clearRoutePreview: function() {
        var pmap = MapFacade.getMap('dashMap') || MapFacade.getMap();
        if (!pmap) return;
        if (this.state.routeLines['autoPreview'] && pmap.hasLayer(this.state.routeLines['autoPreview'])) {
            pmap.removeLayer(this.state.routeLines['autoPreview']);
        }
        if (this.state.routeMarkers['autoPreview'] && pmap.hasLayer(this.state.routeMarkers['autoPreview'])) {
            pmap.removeLayer(this.state.routeMarkers['autoPreview']);
        }
    },
    ...
    _clearMgmtPreview: function() {
        var map = this._getActiveMap();
        if (!map) return;
        var self = this;
        Object.keys(this.state.routeLines).forEach(function(k) {
            if (k.indexOf('mgmtPreview_') === 0) {
                if (self.state.routeLines[k] && map.hasLayer(self.state.routeLines[k]))
                    map.removeLayer(self.state.routeLines[k]);
                delete self.state.routeLines[k];
            }
        });
        Object.keys(this.state.routeMarkers).forEach(function(k) {
            if (k.indexOf('mgmtPreview_') === 0) {
                if (self.state.routeMarkers[k] && map.hasLayer(self.state.routeMarkers[k]))
                    map.removeLayer(self.state.routeMarkers[k]);
                delete self.state.routeMarkers[k];
            }
        });
    },
