...
const GeoServerLayers = {
    ...
    _togglePatrolTracks: function(visible) {
        // 通过 patrol-module 的 state 精确切换巡护相关图层
        if (typeof Patrol !== 'undefined' && Patrol.state) {
            var st = Patrol.state;
            // 轨迹线
            if (st.routeLines) Object.values(st.routeLines).forEach(function(line) {
                if (line && line.setStyle) line.setStyle({ opacity: visible ? 0.85 : 0 });
            });
            // 实时轨迹线
            if (st.realtimeTrackLines) Object.values(st.realtimeTrackLines).forEach(function(track) {
                var line = track && track.line;
                if (line && line.setStyle) line.setStyle({ opacity: visible ? 0.85 : 0 });
            });
            // 起终点标记组
            if (st.routeMarkers) Object.values(st.routeMarkers).forEach(function(group) {
                if (group && group.setOpacity) group.setOpacity(visible ? 1 : 0);
            });
            // 实体标记点（护林员/无人机图标）
            if (st.entityMarkers) Object.values(st.entityMarkers).forEach(function(m) {
                if (m && m.setOpacity) m.setOpacity(visible ? 1 : 0);
            });
        }
    },
    ...
    _addMarkersToMap(map) {
        ...
        // 火情图标：根据等级大小不同，高等级加闪烁动画
        function _fireIcon(level) {
            var sz = level === '高' ? 48 : level === '中' ? 36 : 28;
            var cls = (level === '高') ? 'fire-icon-high' : '';
            return L.divIcon({
                html: '<img src="./fire.png" style="width:' + sz + 'px;height:' + sz + 'px;" class="' + cls + '"/>',
        ...
        Object.keys(firePoints).forEach(key => {
            ...
            var level = f.riskLevel || '低';
            var icon = _fireIcon(level);
            const m = L.marker([f.lat, f.lng], { icon: icon }).addTo(fGroup).addTo(allGroup)
                .bindTooltip((f.name || f.id) + ' · ' + (level || '') + '级火情');
    ...
    addSingleFirePoint(fire) {
        ...
        // 在告警列表最上方插入新火情告警
        var wl = document.getElementById('warningList');
        if (wl) {
            var now = new Date();
            var time = String(now.getHours()).padStart(2,'0') + ':' + String(now.getMinutes()).padStart(2,'0');
            var item = document.createElement('div');
            item.className = 'warning-item high';
            item.setAttribute('data-type', '火情告警');
            item.setAttribute('data-time', time);
            item.innerHTML = '<div class="warning-level"><span class="warning-badge high"></span>火情告警</div>' +
                '<div class="warning-content">' + (fire.description || fire.name || '新火情 #' + fire.id) + '</div>' +
                '<div class="warning-time">' + time + '</div>';
            wl.insertBefore(item, wl.firstChild);
        }
    },
