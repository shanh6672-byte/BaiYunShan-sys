...
(function () {
    ...
    function init(geoJSON) {
        ...
        if (boundaryGCJ && boundaryGCJ.features) {
            ...
            console.log('[ExpLayer] SHP边界图层已创建（GCJ-02转换后）');
        }
        ...
        var NAME_MAP = {
            forestBoundary: '林场边界', subCompartments: '林班小班', rangers: '护林员位置',
            drones: '无人机位置', patrolRoutes: '巡护轨迹', fires: '火情点', diseases: '虫害区'
        };
        ...
        document.querySelectorAll('#businessLayerGroup [data-layer]').forEach(function (el) {
            if (el.type === 'checkbox') {
                ...
                el.addEventListener('change', function () {
                    ...
                    var layer = businessLayers[k];
                    if (!layer) return;
                    var maps = getMapInstances();
                    if (this.checked) {
                        maps.forEach(function (m) { m.addLayer(layer); });
                        console.log('[LayerControl] ' + NAME_MAP[k] + ' 显示');
                    } else {
                        maps.forEach(function (m) { m.removeLayer(layer); });
        ...
        if (ct && !document.getElementById('experimental-legend')) {
            ...
            leg.style.cssText = 'position:absolute;bottom:12px;right:12px;z-index:800;background:rgba(10,22,40,0.92);border:1px solid rgba(79,195,247,0.3);border-radius:8px;padding:10px 14px;font-size:12px;color:#c8d6e5;backdrop-filter:blur(4px);';
